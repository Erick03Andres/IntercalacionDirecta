 ...
 * @author David
 */
public class OrdenamientoBalanceadoFechas {
    ...
    static final String tope="31/12/2500";

    
    public OrdenamientoBalanceadoFechas() throws FileNotFoundException, ParseException {
     ...
     public static void mezclaEqMple() throws FileNotFoundException, IOException, ParseException
    {
        int i, j, k, k1, t;
        String anterior="";
        int [] c=new int[N];
                    ...
                    while(!finDeTramos(actvs, k1))
                    {
                        ...
                        anterior = r[n];
                            ...
                            if(flujoEntradaActual.readRecord()){
                                ...
                                anteriorFecha =formateador.parse(anterior);
                                claveFecha =formateador.parse(r[n]);
                                if(anteriorFecha.compareTo(claveFecha)>0){
                                    actvs[n]=false;
                                }
                            }
    ...
    public static int distribuir() throws IOException, ParseException
    {
        int  j, nt;
        //int clave;
        //String anterior="";
        ...
        SimpleDateFormat formateador = new SimpleDateFormat("dd/MM/yyyy");
        anterior=formateador.parse("01/01/1200");
        clave=formateador.parse("01/01/1201");
        //anterior="a";
        //clave=anterior+1;
        ...
        //{   
            while(flujo.readRecord())
            {
                ...
                clave=(formateador.parse( flujo.get(3)));
                getClave=flujo.get(3);
                get1=flujo.get(0);
                get2=flujo.get(1);
                get3=flujo.get(2);
    ...
    private static int minimo(String []r, boolean[]activo, int n) throws ParseException
    {
        int i, indice;
        Date m;
        SimpleDateFormat formateador = new SimpleDateFormat("dd/MM/yyyy");
        ...
        for(int j=0;j<n;j++){
            aux[j]=formateador.parse(r[j]);
        }
        
        i=indice=0;
        //m=tope+1
        m=formateador.parse(tope);
        //m=tope;
        
        for(;i<n; i++)
        {
            if(activo[i]&&aux[i].compareTo(m)<0)
            {
                m=aux[i];
                indice=i;
            }
    ...
    static void escribir(File f)
    {
        ...
        try
        {
            ...
            {   
                ...
                System.out.println(flujo.get(3) + " ");
               // if(k%19==0)
                   // System.out.println();
                is_end = flujo.readRecord();
