/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package mezcladirectajoyanescsvlisto;

import java.io.EOFException;
import java.io.File;
import java.io.FileReader;
import java.io.FileWriter;
import java.io.IOException;
import com.csvreader.CsvReader;
import com.csvreader.CsvWriter;
import interfaz.Prueba;

/**
 *
 * @author David Bunay
 */
public class MezclaDirectaNumeros {
     
    public MezclaDirectaNumeros() throws IOException {
        File fich = new File(Prueba.nombreArchivo);
        System.out.println("Ordenando Numeros");
        
        mezclaDirecta(fich);
        System.out.println("Archivo Ordenado Numeros");
    }
    
    public  void escribir(File f) throws IOException{
        String sCadena;
        CsvReader bf = new CsvReader(new FileReader(f), ',');
        boolean is_end = bf.readRecord();
        while(is_end){
            int numero= Integer.parseInt(bf.get(0));
            
            System.out.println(numero);
            is_end = bf.readRecord();
        }   
    }
     
    public  void mezclaDirecta(File f) throws IOException {
        int longSec;
        int numReg = 0;
        //int numReg;
        String cad;
        
        CsvReader bf = new CsvReader(new FileReader(f), ',');
        boolean is_end = bf.readRecord();
        /// Leer el numero de registros
        while(is_end){
            numReg++;
            is_end = bf.readRecord();
        }
        
        
 
        File f1 = new File("ArchivoAux1.csv");
        File f2= new File("ArchivoAux2.csv");
        /* número de registros se obtiene dividiendo el tamaño
        del archivo por el tamaño del registro: 4.
        */
        //numReg = (int)(f.length()/4)-1;
        System.out.println("Num registros:->"+numReg);
        longSec = 1;
        
        while (longSec < numReg)
        {
            distribuir(f, f1, f2, longSec, numReg);
            //System.out.println("ENTRANDO..");
            mezclar(f1, f2, f, longSec, numReg);
            longSec *= 2;
            
        }
    }
    public void distribuir(File f, File f1, File f2,int longSec, int numReg) throws IOException{
        int numSec, resto, i;
        
        CsvReader bf = new CsvReader(new FileReader(f), ',');
        CsvWriter pw1 = new CsvWriter(new FileWriter(f1),',');
        CsvWriter pw2 = new CsvWriter(new FileWriter(f2), ',');
        
        numSec = numReg /(2*longSec);
        resto = numReg %(2*longSec);
       
        //Escribe los datos en los arc auxiliares
        for (i = 1; i <= numSec; i++)
        {
            subSecuencia(bf, pw1, longSec);
            subSecuencia(bf, pw2, longSec);
        }
        /*
        Se procesa el resto de registros del archivo
        */
        if (resto > longSec)
            resto -= longSec;
        else
        {
            longSec = resto;
            resto = 0;
        }
        //Escribe los datos sobrantes 
        subSecuencia(bf, pw1, longSec);
        subSecuencia(bf, pw2, resto);
        bf.close();
        pw1.close();
        pw2.close();
        
    }
    
     public void subSecuencia(CsvReader f, CsvWriter t,int longSec) throws IOException
    {   
        String clave;
      
        for (int j = 1; j <= longSec; j++)
        {
            boolean is_end = f.readRecord();
            clave=f.get(0);
            //System.out.println("PRIMER VALOR ENTERO:"+clave);
           
            t.write(clave);
            t.write(f.get(1));
            t.write(f.get(2));
            t.write(f.get(3));
            t.endRecord();
        
        }
    }
     
     
   public void mezclar(File f1, File f2, File f,int lonSec, int numReg) throws IOException
  {
        
        int numSec, resto, i, j, k;
        int clave1 =0,clave2 = 0;
        String getClave1="",getClave2="",getClave3="";
        numSec = numReg /(2*lonSec); // número de subsecuencias
        resto = numReg %(2*lonSec);
   
       
        
        CsvReader bf1 = new CsvReader(new FileReader(f1), ',');
        CsvReader bf2 = new CsvReader(new FileReader(f2), ',');
        CsvWriter pw = new CsvWriter(new FileWriter(f), ',');
        
        //claves iniciales
       
            
        boolean is_end =bf1.readRecord();
        String primerValor=bf1.get(0);
        clave1=Integer.parseInt(bf1.get(0));
       
        boolean is_end2 = bf2.readRecord();
        clave2=Integer.parseInt(bf2.get(0));
           
            //bucle para controlar todo el proceso de mezcla
            for (int s = 1; s <= numSec+1; s++)
            {
                int n1, n2;
                n1 = n2 = lonSec;
                
                //Restan datos no procesado
                if (s == numSec+1)
                { // proceso de subsecuencia incompleta
                    if (resto > lonSec)
                        n2 = resto - lonSec;
                    else
                    {
                        n1 = resto;
                        n2 = 0;
                    }
                }
                i = j = 1;
                //n longitud de sub-secuencias 
                while (i <= n1 && j <= n2)
                {
                    int clave;
                    //Realiza la comparacion de quien es menor 
                    if (clave1 < clave2)
                    {
                        clave = clave1;
                        getClave1=bf1.get(1);
                        getClave2=bf1.get(2);
                        getClave3=bf1.get(3);
                        
                        try {
                            //Toma el siguiente elemento del archivo auxiliar 1
                              boolean auxend1=bf1.readRecord();
                              String clave1aux= bf1.get(0);
                              
                            if(auxend1!=false)
                                clave1= Integer.parseInt(clave1aux);
                        }
                        catch(EOFException e){;}
                        i++;
                    }
                    else
                    {
                        clave = clave2;
                        getClave1=bf2.get(1);
                        getClave2=bf2.get(2);
                        getClave3=bf2.get(3);
                        try {
                            
                            //Toma el siguiente elemento del archivo auxiliar 2
                             boolean auxend2=bf2.readRecord();
                             String clave2aux= bf2.get(0);
                             
                            if(auxend2 !=false)
                                clave2= Integer.parseInt(clave2aux);
                        }
                        catch(EOFException e){;}
                        j++;
                    }
                    
                    String clavem= String.valueOf(clave);
                    //Escribe sobre el archivo original el valor  comparado
                    pw.write(clavem);
                    pw.write(getClave1);
                    pw.write(getClave2);
                    pw.write(getClave3);
                    pw.endRecord();
                   

                }
                
                //Los registros no procesados se escriben directamente
                
                for (k = i; k <= n1; k++)
                {   
                    
                    String clavem= String.valueOf(clave1);
                    getClave1=bf1.get(1);
                    getClave2=bf1.get(2);
                    getClave3=bf1.get(3);
                    pw.write(clavem);
                    pw.write(getClave1);
                    pw.write(getClave2);
                    pw.write(getClave3);
                    pw.endRecord();
                    
                    try {
                    
                            boolean auxend1=bf1.readRecord();
                            String clave1aux= bf1.get(0);
                            //
                            if(auxend1!=false)
                                clave1= Integer.parseInt(clave1aux);
                    }
                    catch(EOFException e){;}
                }
                for (k = j; k <= n2; k++)
                {
                    //System.out.println("\tordenando3:"+clave2);
                    String clavem= String.valueOf(clave2);
                    getClave1=bf2.get(1);
                    getClave2=bf2.get(2);
                    getClave3=bf2.get(3);
                    pw.write(clavem);
                    pw.write(getClave1);
                    pw.write(getClave2);
                    pw.write(getClave3);
                    pw.endRecord();
                    
                    try {
                        
                        boolean auxend2=bf2.readRecord();
                        String clave2aux= bf2.get(0);
                        if(auxend2!=false)
                           clave2= Integer.parseInt(clave2aux);
                    }
                    catch(EOFException e){;}
                }
            }
        bf1.close();
        bf2.close();
        pw.close();
        
}
}
