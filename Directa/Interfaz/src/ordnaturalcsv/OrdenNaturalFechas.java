 ...
 * @author David
 */
public class OrdenNaturalFechas {
    public OrdenNaturalFechas() throws Exception{
    ...
    public boolean particion(String nombreArchivo, String archivo1, String archivo2) throws ParseException {
        ...
        try {
            ...
            while (Archivo.readRecord()) {
                ...
                fecha_leida1 =  formateador.parse(actual);
                fecha_leida2 =  formateador.parse(anterior);
                
                //Cambio de secuencia. Manipulacion del indice del arreglo de archivos
                if (fecha_leida2.compareTo(fecha_leida1) > 0) {
                    //System.out.println("ENTRA");
                    //System.out.println("Actual->"+actual+"Anterios->"+anterior);
    ...
    public void fusion(String nombreArchivo, String archivo1, String archivo2) throws ParseException {
        //Variables para almacenar los datos de los archivos
        //que contienen las particiones
        String[] actual = new String[2];
        String[] anterior = new String[2];
        Date fecha_actual0;
        Date fecha_anterior0;
        Date fecha_actual1;
        Date fecha_anterior1;
        ...
        try {
            ...
            while (contAux1 > 0 && contAux2 > 0) {
                ...
                SimpleDateFormat formateador = new SimpleDateFormat("dd/MM/yyyy");
                fecha_actual0 =  formateador.parse(actual[0]);
                fecha_anterior0 =  formateador.parse(anterior[0]);
                fecha_actual1 =  formateador.parse(actual[1]);
                fecha_anterior1 =  formateador.parse(anterior[1]);

                // mezclamos las dos secuencias hasta que una acaba
                while (fecha_anterior0.compareTo(fecha_actual0) <= 0 &&  fecha_anterior1.compareTo(fecha_actual1) <= 0)
                {
                    //Systfecha_anterior0 =  formateador.parse(anterior[0]);em.out.println("Inifinito");
                    indexArchivo = (fecha_actual0.compareTo(fecha_actual1) <= 0) ? 0 : 1;
               ...
               // indexArchivo = indexArchivo == 0 ? 1 : 0;
                ...
                if(indexArchivo == 0 ){
                    while (fecha_anterior1.compareTo(fecha_actual1) <= 0) {
                        Archivo.write(get21);
                        Archivo.write(get22);
                        Archivo.write(get33);
                        Archivo.write(actual[1]);
                ...
                if(indexArchivo == 1){
                    while (fecha_anterior0.compareTo(fecha_actual0) <= 0) {
                        Archivo.write(get11);
                        Archivo.write(get12);
                        Archivo.write(get13);
                        Archivo.write(actual[0]);
